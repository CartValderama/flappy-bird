extends Node

const SCORES_PATH: String = "user://flappy.tres"

var _high_score: int = 0
var high_score: int:
	get: return _high_score
	set(value):
		if value > _high_score:
			_high_score = value
			save_high_score()

func _ready() -> void:
	load_high_score()

func load_high_score() -> void:
	if ResourceLoader.exists(SCORES_PATH):
		var high_score_resource = load(SCORES_PATH)
		if high_score_resource:
			_high_score = high_score_resource.high_score

func save_high_score() -> void:
	var high_score_resource: HighScoreResource = HighScoreResource.new()
	high_score_resource.high_score = _high_score
	ResourceSaver.save(high_score_resource, SCORES_PATH)
